...
instance Profunctor (RecordFields) where
  dimap f g (RecordFields rr) = RecordFields $ hoistAlt (lmap f) $ fmap g rr

-- | Map a function over all the field names
overFieldNames :: (Text -> Text) -> RecordFields from a -> RecordFields from a
overFieldNames f (RecordFields rr) = RecordFields (hoistAlt ((over fieldNameL f)) rr)
...
union' :: NonEmpty (UnionTag v from) -> TypedSchema from
...
extractSchema = go
  where
    go :: TypedSchemaFlex from to -> NonEmpty Schema
    go (TNamed n sc _ _)= Named n <$> go sc
    go (TPrim n _ _)    = pure $ Prim n
    go (TTry _ sc _)    = go sc
    go (TOneOf scc)     = pure $ OneOf $ go =<< scc
    go (TAllOf scc)     = go =<< scc
    go (TEmpty{})       = pure $ Empty
    go (TEnum opts  _)  = pure $ Enum (fst <$> opts)
    go (TArray sc _ _)  = Array <$> go sc
    go (TMap sc _ _)    = StringMap <$> go sc
    go (RecordSchema rs) = fromList $
      foldMap (\x -> pure (Record (fromList x))) (extractFieldsMu rs)

    -- | Extract all the field groups (from alternatives) in the record
    extractFieldsMu :: RecordFields from to -> [ [(Text, Field)] ]
    extractFieldsMu = runAlt_ (\x -> (:[]) <$> NE.toList (extractField x)) . getRecordFields where
      ...
      extractField (RequiredAp n sc  ) = (\s -> (n, (`Field` True) s)) <$> (go sc)
      extractField (OptionalAp n sc _) = (\s -> (n, (`Field` False) s)) <$> (go sc)
...
encodeToWith sc target =
 ...
 where
  ...
  go _ ctx other src   = failWith ctx (SchemaMismatch (NE.head $ extractSchema other) src)
...
decodeFromWith sc source = (runExcept .) <$> runExcept (go [] [] sc (source))
  where
    ...
    go :: [(SchemaName, Except D (Value -> Except D Void))]
       -> Trace
       -> TypedSchemaFlex from a
       -> Schema
       -> Except D (Value -> Except D a)
    go _nv _tx (TEmpty a) _ = pure $ const $ pure a
    go env ctx (TNamed n sc tof _) (Named n' s) | n == n' =
      case lookup n env of
        Just sol -> do
          -- TODO understand why this delay is necessary
          ...
          f RequiredAp{..} =
            case Map.lookup fieldName src of
              ...
              Just srcField -> do
                ...
                case runExcept $ go env (fieldName:ctx) fieldTypedSchema (fieldSchema srcField) of
                  Left  _ -> empty
    ...
    go _nv ctx s            src = failWith ctx (SchemaMismatch (NE.head $ extractSchema s) src)
